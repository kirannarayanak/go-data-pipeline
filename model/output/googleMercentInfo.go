...
func WriteRSSFeedToFile(adItems []AdItem) error {
	...
	for _, ad := range adItems {
		items = append(items, Item{
			ID:           ad.ID,
			Title:        ad.Title,
			Description:  ad.Description, // No need to encode this into HTML
			Link:         ad.Link,
			ImageLink:    ad.ImageLink,
			Brand:        ad.Brand,
			Price:        ad.Price,        // Ensure price is in "2999.00 AED" format
			Availability: ad.Availability, // "in stock" or "out of stock"
			GTIN:         ad.GTIN,
		})
	}

	// Create the full RSS feed structure
	rssFeed := RSS{
		Version: "2.0",
